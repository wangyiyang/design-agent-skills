...
export function postProcess(html, options = {}) {
  ...
  // Replace external links with terminal-green spans.
  result = result.replace(
    /<a href="([^"]+)" data-external="true"[^>]*>([^<]*)<\/a>/g,
    `<span style="color:${VI.colors.terminalGreen};">$2</span>`
  );
  ...
  result = `<section style="max-width:680px;margin:0 auto;padding:2em 1.5em;background:${VI.colors.offWhite};color:${VI.colors.carbonBlack};font-family:${VI.fonts.sans};font-size:${VI.sizes.body};line-height:${VI.lineHeights.body};">\n${result}\n</section>`;
