...
#
# 铁律背景（2026-07-17 立下）：yy-design 做设计前必须①资产协议(brand-spec.md)
...
yy-design 铁律：长片渲染前必须完成「三方向真实视觉给用户选择」（或用户明示豁免），并把选择/豁免记录写入项目目录的 direction-approved.md（含：展示了哪几版、截图路径、用户的选择原话）。
